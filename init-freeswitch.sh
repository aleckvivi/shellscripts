...
if [ ! -n "$1" ] ;then
  sed -i "s/36.133.135.4/$1/g" ./freeswitch_conf/vars.xml
  sed -i "s/36.133.135.4/$1/g" ./freeswitch_conf/internal.xml
  sed -i "s/36.133.135.4/$1/g" ./freeswitch_conf/external.xml
  echo "已替换为指定的IP地址:$1"
fi
...
then
  ...
  cp ./freeswitch_conf/modules.conf.xml $2"/autoload_configs"
